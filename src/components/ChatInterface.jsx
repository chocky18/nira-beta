...
export default function ChatInterface() {
  ...
  const resetAll = () => {
    ...
    fetchQuiz(null, newId); // Pass new user_id directly
  };
  ...
  const fetchQuiz = async (answer = null, forcedUserId = null) => {
    try {
      ...
      if (quizDone) {
  // Show loading animation before chat
        setLoadingChat(true);
  ...
  <div className={`chat-wrapper stage-${stage}`}>
    {loadingChat ? (
      // Loading animation after quiz completion
        ...
        <div className="chat-right">
                ...
                <div key={region}>
                  ...
                  <ul>
                    {Object.entries(regionMetrics).map(([metric, value]) => (
                      <li key={metric}>{metric}: {JSON.stringify(value)}</li>
                    ))}
                  </ul>
                </div>
              ))}
          ...
          {quizImages.length > 0 && (
            <div className="response-section">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, 80px)", gap: "8px" }}>
                {quizImages.map((imgUrl, idx) => (
                  <img key={idx} src={imgUrl} alt={`ref-${idx}`} style={{ width: "80px", height: "80px", objectFit: "cover", borderRadius: "6px" }} />
                ))}
              </div>
            </div>
          )}
          ...
          {responseImages.original && (
            <div className="response-section">
              ...
              <img src={`data:image/jpeg;base64,${responseImages.original}`} alt="Original" />
            </div>
          )}
          ...
          {responseImages.enhanced && (
            <div className="response-section">
              ...
              <img src={`data:image/jpeg;base64,${responseImages.enhanced}`} alt="Enhanced" />
            </div>
          )}
              ...
              {Object.entries(responseImages.cv2_filters).map(([region, filters]) => (
                <div key={region}>
                  ...
                  {Object.entries(filters).map(([filterName, base64]) => (
                    <div key={filterName}>
                      <p>{filterName}</p>
                      <img src={`data:image/png;base64,${base64}`} alt={`${region}-${filterName}`} />
                    </div>
                  ))}
                </div>
              ))}
          ...
          {steps.length > 0 && (
            <div className="response-section">
              ...
              <ul>
                {steps.map((step, idx) => (
                  <li key={idx}><strong>{step.stage}</strong>: {step.msg}</li>
                ))}
              </ul>
            </div>
          )}
